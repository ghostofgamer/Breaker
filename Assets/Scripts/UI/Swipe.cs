...
namespace UI
{
    public class Swipe : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            for (int i = 0; i < pos.Length; i++)
            {
                /*Debug.Log("Current posCroll" + scroll_pos);
                        Debug.Log("Position " + pos[i]);
                        Debug.Log("Distance " + distance);
                        Debug.Log("DistancePath " + distance / 2);
                        
                        Debug.Log("Current Distance" + pos[i] + (distance / 2));*/
         
                if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
                {
                    // transform.GetChild(i).localScale = Vector3.Lerp(transform.GetChild(i).localScale, new Vector3(130f, 210f,130f), 0.1f);
                    transform.GetChild(i).localScale = Vector2.Lerp(transform.GetChild(i).localScale, new Vector2(1f, 1f), 0.1f);
                    imageContent.transform.GetChild(i).localScale = Vector3.Lerp(imageContent.transform.GetChild(i).localScale, new Vector2(1.2f, 1.2f), 0.1f);
                    ...
                    for (int j = 0; j < pos.Length; j++)
                    {
                        if (j != i)
                        {
                            ...
                            // transform.GetChild(j).localScale = Vector3.Lerp(transform.GetChild(j).localScale, new Vector3(100f, 100f,100f),0.1f);
                        }
                    }
                }
            }
