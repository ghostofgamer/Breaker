    ...
    void Update()
    {
        ...
        for (int i = 0; i < pos.Length; i++)
        {
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
            {
                ...
                // transform.GetChild(i).localScale = Vector2.Lerp(transform.GetChild(i).localScale, new Vector3(100f, 135f,100f), 0.1f);
                ...
                for (int j = 0; j < pos.Length; j++)
                {
                    if (j != i)
                    {
                        ...
                        // transform.GetChild(j).localScale = Vector2.Lerp(transform.GetChild(j).localScale, new Vector3(80f, 100f,80f), 0.1f);
                    }
                }
            }
        }
